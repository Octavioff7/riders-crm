...
self.addEventListener('notificationclick',e=>{
  ...
  // Si la notificación es de un contacto (cid), abrir directo su ficha.
  const cid=e.notification.data&&e.notification.data.cid;
  const url=cid?('/?c='+encodeURIComponent(cid)):'/';
  e.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(ws=>{
    for(const w of ws){if('focus'in w){if(cid)w.postMessage({abrirCliente:cid});return w.focus();}}
